...
__INGESTPATH__ = os.path.join(PAR_DIRECTORY,"data", "processed", "param_043.parquet")
CLUSTERS=[3,4,8]
...
def ml_model_func_006(param_040):
    """
    model_run: =>param_034.fit(data)
    """
    try:
        data = pd.read_parquet(param_040)
            ...
            for c in CLUSTERS:
                for d in INIT_TYPE:
                    ...
                    with mlflow.start_run() as run:
                        ...
                        # Log the param_034
                        self_predictions=kmeans.ml_model_func_011(data)
                        signature = infer_signature(data,self_predictions)
                        mlflow.sklearn.log_model(kmeans, "param_034", signature=signature)
                        run_dict={}
...
#ml_model_func_006(__INGESTPATH__)
